...
3. THREAT INTELLIGENCE INTERPRETATION RULE:
   - The Threat Intelligence results in the Evidence Package are deterministic ground-truth facts.
   - If an IOC has "match_found": true in the Evidence Package, you MUST report "match_found": true and cite the actual database source (e.g. MalwareBazaar, ThreatFox, URLhaus).
   - If an IOC has "match_found": false in the Evidence Package, you MUST report "match_found": false.
   - In your interpretation, avoid claiming that a match "validates the malicious nature of the executed file" or proves compromise; describe it strictly as threat-intelligence evidence associated with the indicator requiring analyst review to correlate with observed host activity.
...
Respond ONLY with the JSON object, no Markdown backticks or commentary."""
    ...
    def generate_fallback_report(
        ...
        return self.reconcile_threat_intelligence(report, evidence_package)

    @classmethod
    def reconcile_threat_intelligence(
        cls,
        parsed_report: Dict[str, Any],
        evidence_package: Dict[str, Any],
    ) -> Dict[str, Any]:
        """
        Reconcile threat intelligence findings deterministically against the Evidence Package.

        Guarantees that deterministic backend evidence (queried_ioc, match_found, source,
        ioc_type, threat_type) is authoritative and CANNOT be overridden by LLM hallucination.
        The LLM's interpretation is preserved as explanatory commentary, but any contradiction
        (e.g. claiming no-match when a database match exists) is corrected to reflect the factual evidence.
        """
        if not isinstance(parsed_report, dict):
            return parsed_report

        det_ti_list = evidence_package.get("threat_intelligence", []) if isinstance(evidence_package, dict) else []
        if not det_ti_list:
            return parsed_report

        # Map LLM findings by lowercase IOC for interpretation lookup
        llm_findings = parsed_report.get("threat_intelligence_findings", [])
        llm_by_ioc: Dict[str, Dict[str, Any]] = {}
        if isinstance(llm_findings, list):
            for item in llm_findings:
                if isinstance(item, dict):
                    ioc_key = str(item.get("ioc") or item.get("queried_ioc") or "").strip().lower()
                    if ioc_key:
                        llm_by_ioc[ioc_key] = item

        reconciled: List[Dict[str, Any]] = []
        for det in det_ti_list:
            q_ioc = det.get("queried_ioc", "")
            norm_key = str(q_ioc).strip().lower()
            det_match_found = bool(det.get("match_found", False))
            matches = det.get("matches", []) or []
            first_match = matches[0] if (matches and isinstance(matches[0], dict)) else {}

            if det_match_found and first_match:
                det_source = first_match.get("source") or "Local Threat Intelligence Database"
                det_ioc_type = first_match.get("ioc_type") or det.get("ioc_type", "unknown")
                det_threat_type = first_match.get("threat_type") or ""
                det_malware = first_match.get("malware") or ""
            else:
                det_source = "Local Threat Intelligence Database"
                det_ioc_type = det.get("ioc_type", "unknown")
                det_threat_type = ""
                det_malware = ""

            llm_item = llm_by_ioc.get(norm_key, {})
            llm_interp = str(llm_item.get("interpretation") or "").strip()

            if det_match_found:
                # Check for overly strong claims or contradiction in LLM interpretation
                has_overly_strong_claim = any(
                    phrase in llm_interp.lower()
                    for phrase in (
                        "validates the malicious nature",
                        "validates malicious",
                        "proves compromise",
                        "definitely executed",
                ...
                contradicts = (
                    not llm_interp
                    or "not found" in llm_interp.lower()
                    or "no match" in llm_interp.lower()
                    or "no threat intelligence" in llm_interp.lower()
                    or "lacks prior cataloging" in llm_interp.lower()
                )

                if det_source == "MalwareBazaar":
                    ...
                    if has_overly_strong_claim or contradicts or not llm_interp:
                        details = []
                        if det_threat_type:
                            details.append(f"threat type: {det_threat_type}")
                        if det_malware:
                            details.append(f"malware: {det_malware}")
                        detail_str = f" ({', '.join(details)})" if details else ""
                        interpretation = (
                            f"Indicator matches a record in {det_source}{detail_str}. "
                            f"This provides threat-intelligence evidence associated with the indicator; "
                            f"analyst review is required to correlate it with observed host activity."
                        )
                    else:
                        interpretation = llm_interp
            else:
                # Deterministic no-match: ensure LLM does not falsely claim a positive match
                falsely_claimed_match = (
                    not llm_interp
                    or ("found" in llm_interp.lower() and "not found" not in llm_interp.lower())
                )
                # Check for RFC 1918 internal IPs
                is_internal = any(
                    norm_key.startswith(pfx)
                    for pfx in ("10.", "192.168.", "172.16.", "172.17.", "172.18.", "172.19.", "172.20.", "172.21.", "172.22.", "172.23.", "172.24.", "172.25.", "172.26.", "172.27.", "172.28.", "172.29.", "172.30.", "172.31.")
                )
                if is_internal:
                    if falsely_claimed_match or "rfc" not in llm_interp.lower():
                        interpretation = "Internal RFC 1918 IP addresses are protected infrastructure and are not expected to appear in external threat feeds; no match is expected or relevant."
                    else:
                        interpretation = llm_interp
                else:
                    if falsely_claimed_match or not llm_interp or ("does not mean" not in llm_interp.lower() and "benign" not in llm_interp.lower()):
            ...
            finding = {
                ...
                "queried_ioc": q_ioc,
                "source": det_source,
                "match_found": det_match_found,
                "ioc_type": det_ioc_type,
                "threat_type": det_threat_type,
                "interpretation": interpretation,
            }
            reconciled.append(finding)

        parsed_report["threat_intelligence_findings"] = reconciled
        return parsed_report
    ...
    def reason_over_evidence(
        ...
        try:
            ...
            # Reconcile threat intelligence findings deterministically against evidence package
            parsed_report = self.reconcile_threat_intelligence(parsed_report, evidence_package)
